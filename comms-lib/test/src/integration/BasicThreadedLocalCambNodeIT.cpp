...
// Helper classes for the BasicThreadedLocalCamNodeIT
namespace BasicThreadedLocalNodeITTest {
    ...
    struct TestMessageProcessor: cadf::comms::MessageProcessor<T> {
            ...
            virtual void verifyMessageReceived(T *expected) {
                if (expected == NULL)
                    ...
                    BOOST_CHECK(NULL != receivedMsg);
                    BOOST_CHECK(expected != receivedMsg);
                    BOOST_CHECK(expected->equals(receivedMsg));
                    delete (receivedMsg);
                }
    ...
    struct TestFixture {
            ...
            TestFixture() : threadPool(), bus(&threadPool), msgRegistry(), msgFactory(512), node1_1Connection(1, 1, &msgFactory), node1_2Connection(1, 2, &msgFactory),
                    node2_1Connection(2, 1, &msgFactory), node2_2Connection(2, 2, &msgFactory) {
            ...
            void verifyMessagesReceived(TestMessage1 *n1_1tm, TestMessage2 *n1_1tm2, TestMessage1 *n1_2tm, TestMessage2 *n1_2tm2, TestMessage1 *n2_1tm, TestMessage2 *n2_1tm2, TestMessage1 *n2_2tm,
                ...
                // Since we're dealing with a threaded message sender, the message will be sent asynchronously. Need to wait for the message to make it through.
                // 10 ms seems to be sufficient.
...
BOOST_AUTO_TEST_SUITE(ThreadedLocalNodeIT_Test_Suite)

/**
 * Verify that an exception is thrown when trying to send a message that wasn't registered with the message factory
 */
    BOOST_FIXTURE_TEST_CASE(SendMessagesNotRegisteredWithFactoryConnectedTest, BasicThreadedLocalNodeITTest::TestFixture) {
        connectNodes();
    ...
    /**
     * Verify that attempting to send a message when disconnected will fail.
     */
     ...
     */
    BOOST_FIXTURE_TEST_CASE(SendMessagesNoProcessorsAddedTest, BasicThreadedLocalNodeITTest::TestFixture) {
        connectNodes();
     ...
     */
    BOOST_FIXTURE_TEST_CASE(SendMessagesOneProcessorAddedTest, BasicThreadedLocalNodeITTest::TestFixture) {
        // Register the processor on node 2
        node1_2->addProcessor(&node1_2TestMessageProcessor);
        // Multiple connections don't impact functionality
        connectNodes();
        connectNodes();
        connectNodes();
        connectNodes();
     ...
     * Verify that when a processor is registered that message is delivered
     */
    BOOST_FIXTURE_TEST_CASE(SendMessagesManyProcessorsAddedBroadcastToAllTest, BasicThreadedLocalNodeITTest::TestFixture) {
        connectNodes();
    ...
    /**
     * Verify that disconnecting the node will stop the sending of messages
     */
    BOOST_FIXTURE_TEST_CASE(DisconnectStopsSendingTest, SendMessagesManyProcessorsAddedBroadcastToAllTest) {
        // Invoke the SendMessagesManyProcessorsAddedTest test
        SendMessagesManyProcessorsAddedBroadcastToAllTest::test_method();

        // Disconnecting prevents the sending of messages
        disconnectNodes();
        SendMessagesNotRegisteredWithFactoryDisconnectedTest().test_method();

        // Repeated disconnects don't change functionality
        disconnectNodes();
        SendMessagesNotRegisteredWithFactoryDisconnectedTest().test_method();
        disconnectNodes();
        SendMessagesNotRegisteredWithFactoryDisconnectedTest().test_method();

        // Reconnect and send messages again
        SendMessagesManyProcessorsAddedBroadcastToAllTest::test_method();
        // Repeatedly connecting doesn't change functionality
        SendMessagesManyProcessorsAddedBroadcastToAllTest::test_method();
        SendMessagesManyProcessorsAddedBroadcastToAllTest::test_method();
        SendMessagesManyProcessorsAddedBroadcastToAllTest::test_method();
    }
