...
namespace cadf::comms::json {
    ...
    class JSONExtractor {
        ...
        private:
             ...
             */
            template<typename T>
            bool tryGetValueFromPOD(const cadf::comms::json::JSONValue *value, const std::string &name, std::vector<T>& result) const {
                // Check if this is the appropriate type of node
                const JSONPODValue *pod = dynamic_cast<const cadf::comms::json::JSONPODValue*>(value);
                if (!pod)
                    return false;
             ...
             */
            template<typename T>
            bool tryGetValueFromNode(const cadf::comms::json::JSONValue *value, const std::string &name, std::vector<T>& result) const {
