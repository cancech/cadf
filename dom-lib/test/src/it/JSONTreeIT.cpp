...
namespace JSONTreeTest {
    ...
    void createAndAssertNode(std::string name, std::string key, const T &value, const std::string &expectedStr) {
        cadf::dom::JSONPODValue podVal(key, value);
        BOOST_CHECK_EQUAL(key, podVal.getName());
        cadf::dom::JSONNode node(name);
        BOOST_CHECK_EQUAL(name, node.getName());
        node.setValue(&podVal);
        cadf::dom::JSONValue *actual = node.getValue(key)[0];
        BOOST_CHECK_EQUAL(&podVal, actual);
        BOOST_CHECK_EQUAL(expectedStr, node.toString());
    }

    template<typename T>
    T getValueFromNode(const cadf::dom::JSONNode* node, const std::string &key) {
        const cadf::dom::JSONValue *val = node->getValue(key)[0];
        return static_cast<const cadf::dom::JSONPODValue*>(val)->getValue<T>();
    }

    const cadf::dom::JSONNode* getNestedNodeFromNode(const cadf::dom::JSONNode* node, const std::string &key) {
        const cadf::dom::JSONValue *val = node->getValue(key)[0];
        return static_cast<const cadf::dom::JSONNode*>(val);
    }
}
 ...
 */
    ...
    BOOST_AUTO_TEST_CASE(CreateNodeWithMultipleValuesTest) {
        cadf::dom::JSONNode node("Test Node");
        cadf::dom::JSONPODValue val1("val1", 1);
        std::string val2Val = "Value 2";
        cadf::dom::JSONPODValue val2("val2", val2Val);
        cadf::dom::JSONPODValue val3("val3", 5.40);
        cadf::dom::JSONPODValue val4("val4", -5);
        node.setValue(&val1);
        node.setValue(&val2);
        node.setValue(&val3);
        node.setValue(&val4);
    ...
    BOOST_AUTO_TEST_CASE(CreateSimpleTreeTest) {
        // Build the tree
        cadf::dom::JSONNode root("root");

        cadf::dom::JSONPODValue intVal1("int1", 123);
        cadf::dom::JSONPODValue intVal2("int2", 234);
        cadf::dom::JSONPODValue intVal3("int3", 345);
        root.setValue(&intVal1);
        root.setValue(&intVal2);
        root.setValue(&intVal3);

        std::string val = "This is a string!";
        cadf::dom::JSONPODValue stringVal("String value", val);
        root.setValue(&stringVal);

        cadf::dom::JSONNode parentNode("Parent Node");
        cadf::dom::JSONPODValue parentIntVal("int", 123);
        cadf::dom::JSONPODValue parentDblVal("double", 1.23);
        cadf::dom::JSONPODValue parentLongVal("long", 123123123);
        ...
        const cadf::dom::JSONNode *subNode = JSONTreeTest::getNestedNodeFromNode(&root, "Parent Node");
    ...
    BOOST_AUTO_TEST_CASE(MenuExampleTest) {
        ...
        cadf::dom::JSONNode menu("menu");
        root.setValue(&menu);

        std::string idVal = "file";
        cadf::dom::JSONPODValue id("id", idVal);
        std::string valueVal = "File";
        cadf::dom::JSONPODValue value("value", valueVal);
        menu.setValue(&id);
        menu.setValue(&value);

        cadf::dom::JSONNode menuItem1("menuitem");
        std::string newStr = "New";
        cadf::dom::JSONPODValue menuItem1Val("value", newStr);
        std::string newClickStr = "CreateNewDoc()";
        cadf::dom::JSONPODValue menuItem1Click("onclick", newClickStr);
        menuItem1.setValue(&menuItem1Val);
        menuItem1.setValue(&menuItem1Click);

        cadf::dom::JSONNode menuItem2("menuitem");
        std::string openStr = "Open";
        cadf::dom::JSONPODValue menuItem2Val("value", openStr);
        std::string openClickStr = "OpenDoc()";
        cadf::dom::JSONPODValue menuItem2Click("onclick", openClickStr);
        menuItem2.setValue(&menuItem2Val);
        menuItem2.setValue(&menuItem2Click);

        cadf::dom::JSONNode menuItem3("menuitem");
        std::string closeStr = "Close";
        cadf::dom::JSONPODValue menuItem3Val("value", closeStr);
        std::string closeClickStr = "CloseDoc()";
        cadf::dom::JSONPODValue menuItem3Click("onclick", closeClickStr);
        menuItem3.setValue(&menuItem3Val);
        menuItem3.setValue(&menuItem3Click);


        cadf::dom::JSONNode popup("popup");
    ...
    BOOST_AUTO_TEST_CASE(CarMakeExampleTest) {
        ...
        cadf::dom::JSONPODValue name("name", nameVal);
        cadf::dom::JSONPODValue age("age", 30);
        root.setValue(&name);
        root.setValue(&age);

        cadf::dom::JSONNode car1("cars");
        std::string car1NameStr = "Ford";
        cadf::dom::JSONPODValue car1Name("name", car1NameStr);
        car1.setValue(&car1Name);

        std::string car1Model1Str = "Fiesta";
        cadf::dom::JSONPODValue car1Model1("models", car1Model1Str);
        std::string car1Model2Str = "Focus";
        cadf::dom::JSONPODValue car1Model2("models", car1Model2Str);
        std::string car1Model3Str = "Mustang";
        cadf::dom::JSONPODValue car1Model3("models", car1Model3Str);
        ...
        cadf::dom::JSONNode car2("cars");
        std::string car2NameStr = "BMW";
        cadf::dom::JSONPODValue car2Name("name", car2NameStr);
        car2.setValue(&car2Name);

        std::string car2Model1Str = "320";
        cadf::dom::JSONPODValue car2Model1("models", car2Model1Str);
        std::string car2Model2Str = "X3";
        cadf::dom::JSONPODValue car2Model2("models", car2Model2Str);
        std::string car2Model3Str = "X5";
        cadf::dom::JSONPODValue car2Model3("models", car2Model3Str);
        ...
        cadf::dom::JSONNode car3("cars");
        std::string car3NameStr = "Fiat";
        cadf::dom::JSONPODValue car3Name("name", car3NameStr);
        car3.setValue(&car3Name);

        std::string car3Model1Str = "500";
        cadf::dom::JSONPODValue car3Model1("models", car3Model1Str);
        std::string car3Model2Str = "Panda";
        cadf::dom::JSONPODValue car3Model2("models", car3Model2Str);
